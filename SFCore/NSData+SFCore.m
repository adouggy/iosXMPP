...
static int hexValueOfChar(unichar hexChar)
{
...
SFC_FIX_CATEGORY_BUG_IMPLEMENTATION(NSData_SFCore)


@implementation NSData (SFCore)
...
+ (NSData *)dataFromBytesString:(NSString *)hexString
{
    ...
    if (len % 2 != 0)
    {
        DDLogError(@"hex string length is not an even number");
        return nil;
    }
    len = len / 2;
    if (len != 0)
    {
        ...
        for(int i = 0;i < len;i++)
        {
            bytes[i] = hexValueOfChar([hexString characterAtIndex:i * 2]) * 16 + hexValueOfChar([hexString characterAtIndex:(i * 2 + 1)]);
        }
        NSData *result = [[NSData alloc] initWithBytes:bytes length:len];
        free(bytes);
        return result;
    }
    else
